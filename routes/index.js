...
router.post("/login", function(req, res, next) {
  ...
  firebase
    ...
    .then(userFirebase => {
      mongodb
        .getByUid(userFirebase.user.uid)
        .then(result => {
...
/* POST Mirante route */
router.post("/dashboardMirante", auth.isAuthenticated, function(
  req,
  res,
  next
) {
...
/* POST Contagem route */
router.post("/dashboardContagem", auth.isAuthenticated, function(
  req,
  res,
  next
) {
...
router.get("/dashboard", auth.isAuthenticated, function(req, res, next) {
  ...
  Rent.getAllByStartLocalRodando(unity)
    .then(rentsAguardando => {
      Rent.getAllByStartLocalAguardando(unity)
        .then(rents => {
          Rent.getAllByEndLocalWaiting(unity)
            .then(rentsWaiting => {
              var clientsRunning = [];
      ...
      Client.update(clientId, client)
        .then(() => {
          ...
          for (var i = 0; i < arrayLength; i++) {
            ...
            Equipament.getByNameI(nameeq, i)
              .then(equipament => {
...
router.get("/show/:_id", auth.isAuthenticated, function(req, res, next) {
  ...
  Rent.getById(id)
    .then(rent => {
      ...
      unitPrice = unitPrice.toLocaleString("pt-br", {
        style: "currency",
        currency: "BRL"
      });
      actualPrice = actualPrice.toLocaleString("pt-br", {
        style: "currency",
        currency: "BRL"
      });
      partialPrice = partialPrice.toLocaleString("pt-br", {
        style: "currency",
        currency: "BRL"
      });
      res.render("show", {
...
router.post("/close/:_id", function(req, res, next) {
  ...
  Rent.getById(id)
    .then(rent => {
      // Pontos de Fidelidade
      Client.getDatePointsById(rent.client.id)
        .then(datePoints => {
          var aux = true;
...
/* GET dailyBalance */
router.get("/dailyBalance", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
  ...
  Rent.getAllByDate(date.day, date.month, date.year)
    .then(rents => {
      ...
      let totalUnits = rents.reduce(
        (totalUnits, cur) => totalUnits + cur.quantity,
        0
      );
      Rent.getAllByDateAndEndLocal("Matriz", date.day, date.month, date.year)
        .then(matrizRents => {
          let matrizProfit = matrizRents.reduce(
            (matrizProfit, cur) => matrizProfit + cur.discount,
            0
          );
          let matrizUnits = matrizRents.reduce(
            (matrizUnits, cur) => matrizUnits + cur.quantity,
            0
          );
          Rent.getAllByDateAndEndLocal(
            "Bem-Te-Vi",
            date.day,
            date.month,
            date.year
            ...
            .then(miranteRents => {
              let miranteProfit = miranteRents.reduce(
                (miranteProfit, cur) => miranteProfit + cur.discount,
                0
              );
              let miranteUnits = miranteRents.reduce(
                (miranteUnits, cur) => miranteUnits + cur.quantity,
                0
              );
              Rent.getAllByDateAndEndLocal(
                "Vila Pampulha",
                ...
                .then(vilaRents => {
                  let vilaProfit = vilaRents.reduce(
                    (vilaProfit, cur) => vilaProfit + cur.discount,
                    0
                  );
                  let vilaUnits = vilaRents.reduce(
                    (vilaUnits, cur) => vilaUnits + cur.quantity,
                    0
                  );
                  Rent.getAllByDateAndEndLocal(
                    "Shopping Contagem",
                    ...
                    .then(contagemRents => {
                      let contagemProfit = contagemRents.reduce(
                        (contagemProfit, cur) => contagemProfit + cur.discount,
                        0
                      );
                      let contagemUnits = contagemRents.reduce(
                        (contagemUnits, cur) => contagemUnits + cur.quantity,
                        0
                      );
                      contagemProfit = contagemProfit.toLocaleString("pt-br", {
                        style: "currency",
                        currency: "BRL"
                      });
                      miranteProfit = miranteProfit.toLocaleString("pt-br", {
                        style: "currency",
                        currency: "BRL"
                      });
                      vilaProfit = vilaProfit.toLocaleString("pt-br", {
                        style: "currency",
                        currency: "BRL"
                      });
                      matrizProfit = matrizProfit.toLocaleString("pt-br", {
                        style: "currency",
                        currency: "BRL"
                      });
  ...
  function(req, res, next) {
    ...
    Rent.getAllByDate(date.day, date.month, date.year)
      .then(rents => {
        ...
        Rent.getAllByDateAndStartLocal(
          "Matriz",
          date.day,
          date.month,
          date.year
                  ...
                  .then(vilaRents => {
                    ...
                    Rent.getAllByDateAndStartLocal(
                      ...
                      .then(contagemRents => {
                        ...
                        contagemProfit = contagemProfit.toLocaleString(
                          "pt-br",
                        ...
                        res.render("dailyBalancePrevious", {
                          title: "Balanço Diário",
                          ...req.session,
                          totalProfit,
                          matrizProfit,
...
router.get("/dailyBalance/next", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
  ...
  if (
    date_at.day == date.day &&
    date_at.monthNumber == date.monthNumber &&
    date_at.year == date.year
  ) {
...
/* GET daily Report */
router.get("/dailyReport", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
  ...
  var date = req.session.date;
  Rent.getAllByDateAndEndLocal(daily.endLocal, date.day, date.month, date.year)
    .then(rents => {
      rents.forEach(rent => {
        if (rent.discount != null) {
          rent.discountR$ = rent.discount.toLocaleString("pt-br", {
            style: "currency",
            currency: "BRL"
          });
        }
...
router.post("/dailyReport", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
  ...
  var date = req.session.date;
  Rent.getAllByDateAndEndLocal(endLocal, date.day, date.month, date.year)
    .then(rents => {
      rents.forEach(rent => {
        if (rent.discount != null) {
          rent.discountR$ = rent.discount.toLocaleString("pt-br", {
    ...
    Rent.getById(id)
      .then(rent => {
        if (rent.receivedPrice != null) {
          rent.receivedPriceR$ = rent.receivedPrice.toLocaleString("pt-br", {
            style: "currency",
            currency: "BRL"
          });
        }
        ...
        res.render("dailyReportDetails", {
          title: "Relatório Diário Detalhes",
          ...req.session,
          rent
        });
      })
...
/* GET dailyBalance */
router.get("/monthlyBalance", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
  ...
  Rent.getAllByMonth(date.month, date.year)
    .then(rents => {
      ...
      Rent.getAllByMonthAndEndLocal("Matriz", date.month, date.year)
        .then(matrizRents => {
          let matrizProfit = matrizRents.reduce(
            (matrizProfit, cur) => matrizProfit + cur.discount,
            0
          ...
          Rent.getAllByMonthAndEndLocal("Bem-Te-Vi", date.month, date.year)
            .then(miranteRents => {
              let miranteProfit = miranteRents.reduce(
                (miranteProfit, cur) => miranteProfit + cur.discount,
                0
  ...
  function(req, res, next) {
    ...
    Rent.getAllByMonth(date.month, date.year)
      .then(rents => {
        ...
        Rent.getAllByMonthAndStartLocal("Matriz", date.month, date.year)
          .then(matrizRents => {
            let matrizProfit = matrizRents.reduce(
              (matrizProfit, cur) => matrizProfit + cur.discount,
              0
            ...
            Rent.getAllByMonthAndStartLocal("Bem-Te-Vi", date.month, date.year)
              .then(miranteRents => {
                let miranteProfit = miranteRents.reduce(
                  (miranteProfit, cur) => miranteProfit + cur.discount,
                  0
                ...
                Rent.getAllByMonthAndStartLocal(
                  "Vila Pampulha",
                  date.month,
                  date.year
                )
                  .then(vilaRents => {
                    ...
                    Rent.getAllByMonthAndStartLocal(
                      ...
                      .then(contagemRents => {
                        ...
                        res.render("monthlyBalancePrevious", {
                          title: "Balanço Mensal",
                          ...req.session,
                          totalProfit,
                          matrizProfit,
                    ...
                    .then(vilaRents => {
                      ...
                      Rent.getAllByMonthAndStartLocal(
                        ...
                        .then(contagemRents => {
                          ...
                          miranteProfit = miranteProfit.toLocaleString(
                            "pt-br",
                            { style: "currency", currency: "BRL" }
                          );
                          vilaProfit = vilaProfit.toLocaleString("pt-br", {
...
/* GET daily Report */
router.get("/monthlyReport", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
...
// POST daily Report
router.post("/monthlyReport", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
...
router.get(
  ...
  function(req, res, next) {
    ...
    Rent.getById(id)
      .then(rent => {
        res.render("monthlyReportDetails", {
          title: "Relatório Mensal Detalhes",
          ...req.session,
          rent
        });
      })
...
router.get("/equipamentBalance", auth.isAuthenticated, auth.isMaster, function(
  req,
  res,
  next
) {
      ...
      Rent.getAllByMonth(date.month, date.year)
        .then(rents => {
          ...
          equipaments.forEach(equipament => {
            ...
            equipament.value = equipament.value.toLocaleString("pt-br", {
              style: "currency",
              currency: "BRL"
            });
          ...
          res.render("equipamentBalance", {
            title: "Balanço de Equipamentos",
            ...req.session,
            equipaments,
            date,
            totalUnits,
      ...
      .then(equipaments => {
        Rent.getAllByMonth(date.month, date.year)
          .then(rents => {
            ...
            res.render("equipamentBalancePrevious", {
              title: "Balanço de Equipamentos",
              ...req.session,
              equipaments,
              date,
...
router.post("/pagamentoTotal/::cpf::endLocal", auth.isAuthenticated, function(
  req,
  res,
  next
) {
